...
Donde $\mu_{CM}$ representa la media de las mediciones realizadas con cinta métrica y $\mu_{A}$ representa la media de las mediciones realizadas con antropómetro. 
...
A continuación se presentan los resultados de los test de Shapiro-Wilk y de Lilliefors para la diferencia de las mediciones LRT_CM y LRT_A:
...
Los resultados de dos pruebas estadísticas, Shapiro-Wilk y Lilliefors, mostraron un p-valor menor a $0.05$. Esto significa que, con un nivel de confianza del $95\%$, no podemos concluir que las diferencias entre las mediciones de rodilla - talón tomadas con una cinta métrica y con un antropómetro sigan una distribución normal, y por lo tanto no es posible usar la prueba t para muestras pareadas. Por lo tanto, para analizar las diferencias en los promedios, es necesario usar un tipo de prueba que no dependa de suposiciones sobre la distribución de los datos.

El siguiente paso es evaluar el sistema de hipótesis mediante una prueba no paramétrica: Test de Rangos con Signo de Wilcoxon. Las hipótesis en este caso no son las mismas expuestas anteriormente. El sistema de hipótesis en este caso es el siguiente:

$$ \begin{cases}
H_0: \textrm{La distribución de las diferencias es simétrica alrededor de 0}\\
H_1: \textrm{La distribución de las diferencias no es simétrica alrededor de 0}
\end{cases}$$

La cuál es equivalente al siguiente sistema de hipótesis bajo $H_0$:
...
A continuación se presenta el resultado del test:

```{r}
wilcox.test(datos$LRT_CM, datos$LRT_A, paired = TRUE, alternative = "two.sided", conf.int = TRUE)
```

El resultado del Test de Rangos con Signo de Wilcoxon mostró un p-valor de $2.2 \times 10^{-16}$, lo que indica que en este caso con una confianza del $95\%$ se rechaza la hipótesis nula de que la distribición de las diferencias sea simétrica alrededor de 0. Se concluye que las mediciones de la longitud rodilla - talón hechas con la cinta métrica son significativamente diferentes a las obtenidas con el antropómetro.
...
### Análisis discriminando por sexo
...
Según el boxplot, se puede observar que podría existir una leve diferencia entre las distribuciones de las mediciones de la longitud rodilla maléolo con la pierna a 90° y con la pierna completamente estirada en los adultos mayores de sexo masculino.

```{r}
#| layout-ncol: 2
# Diferencias 90 y R
LRM_dif <- hombres$LRM_90 - hombres$LRM_R
# Como se ve graficamente, parece que hay normalidad
par(mfrow=c(1,2))
...
boxplot(LRM_dif) # hay muchos outliers
```
De acuerdo al histograma, la mediana de las diferencias entre las mediciones de la longitud rodilla maléolo con la pierna a 90° y con la pierna completamente estirada es de 0.5 cm. Además, pareciera que hay simetría en la distribución de las diferencias, por lo que puede haber normalidad.
...
Dado que en ambos test, Shapiro-Wilk y Lilliefors, se obtuvo un p-valor mayor a $0.05$, entonces, con una significancia del $5\%$, hay evidencia estadística de que las diferencias entre las mediciones rodilla-maleolo con la pierna a 90° y con la pierna estirada siguen una distribució normal. Por lo tanto, se hace necesario realizar una prueba t para la media de las diferencias.
...
El resultado de la prueba t mostró un p-valor de $2.2 \times 10^{-16}$, por lo que se rechaza la hipótesis de igualdad de medias. Esto significa que las mediciones de la longitud rodilla-maleolo hechas con la pierna a 90° no son equivalentes a las obtenidas con la pierna estirada, en los adultos mayores de sexo masculino.
...
Según el boxplot, se puede observar que podría existir una diferencia entre las distribuciones de las mediciones de la longitud rodilla maléolo con la pierna a 90° y con la pierna completamente estirada en los adultos mayores de sexo femenino. En este grupo de mujeres se observa que las medianas entre las medidas es más distante que en el grupo de hombres.  
...
boxplot(LRM_dif) 
```
De acuerdo al histograma, la mediana de las diferencias entre las mediciones de la longitud rodilla maléolo con la pierna a 90° y con la pierna completamente estirada es de 1 cm. 
...
Dado que en ambos test, Shapiro-Wilk y Lilliefors, se obtuvo un p-valor menor a $0.05$, entonces, con una significancia del $5\%$, no hay evidencia estadística de que las diferencias entre las mediciones rodilla-maleolo con la pierna a 90° y con la pierna estirada siguen una distribució normal. Por lo tanto, se hace necesario realizar una prueba no parametrica para la media/mediana de las diferencias.
...
Dado que el p-valor es menor a 0.05, se rechaza la hipotesis nula de que la distribución de las diferencias entre las mediciones rodilla-maleolo con la pierna a 90° y con la pierna estirada sea simetrica. Por lo tanto, se realiza el test del signo para muestras pareadas:
...
El resultado de la prueba del signo arrojó un p-valor de $2.2 \times 10^{-16}$, indicando evidencia estadística suficiente para rechazar la hipótesis nula de que la mediana de las diferencias es igual a cero. Por lo tanto, se concluye que las mediciones de la longitud rodilla-maléolo realizadas con la pierna completamente estirada no son equivalentes a las obtenidas con la pierna a $90°$ en el grupo de las adultas mayores.
